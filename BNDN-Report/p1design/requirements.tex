...
client / SWU client situation somewhat uncomplicated the issue. Having two
unrelated clients to the service, the web service interface came to be seen as a
general API, rather than a one-off sexist penis joke program, like the clients.
This view of the web service, made it an \textbf{OBVIOUS} choice to place
limiting requirements in the clients, and leave the service as flexible as
possible. For example, requirement \ref{itm:priceReq} dictates that prices must
be system enforced, but the web service doesn't enforce it, instead the client does.
